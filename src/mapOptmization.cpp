...
class mapOptimization : public ParamServer {
 public:
  ...
  // gps 基准点
  Eigen::Vector3d gpsRefernencePoint;
  bool gpsInit = false;
  ...
  mapOptimization() {
    ...
    pubGPSPath = nh.advertise<nav_msgs::Path>("lio_sam/mapping/gps_path", 1);
    ...
    subRtk = nh.subscribe<sensor_msgs::NavSatFix>(
        rtkTopic, 200, &mapOptimization::rtkHandler, this,
        ros::TransportHints().tcpNoDelay());
  ...
  void rtkHandler(const sensor_msgs::NavSatFix::ConstPtr& rtkMsg) {
    double latitude = rtkMsg->latitude;
    double longitude = rtkMsg->longitude;
    double altitude = rtkMsg->altitude;

    // 转化为utm坐标系
    UTMCoor utmcoor;
    LatLonToUTMXY(latitude / 180.0 * M_PI, longitude / 180.0 * M_PI, 51,
                  utmcoor);
    Eigen::Vector3d gps_utm(utmcoor.x, utmcoor.y, altitude);
    Eigen::Matrix3d gps_covariance;
    for (int i = 0; i < 3; i++)
      for (int j = 0; j < 3; j++)
        gps_covariance(i, j) = rtkMsg->position_covariance[3 * i + j];

    if (!gpsInit) {
      if (gps_covariance(0, 0) < 1 && gps_covariance(1, 1) < 1 &&
          gps_covariance(2, 2) < 1) {
        gpsRefernencePoint = gps_utm;
        gpsInit = true;
      }
    }

    if (!gpsInit) return;
    gps_utm -= gpsRefernencePoint;
    RTKMsg rtk_data;
    rtk_data.timestamp = rtkMsg->header.stamp.toSec();
    rtk_data.raw_data = Eigen::Vector3d(latitude, longitude, altitude);
    rtk_data.pose = gps_utm;
    rtk_data.covariance = gps_covariance;

    rtkQueue.push_back(rtk_data);

    // 更新gps path msg
    geometry_msgs::PoseStamped pose_stamped;
    pose_stamped.header.stamp = rtkMsg->header.stamp;
    pose_stamped.header.frame_id = odometryFrame;
    pose_stamped.pose.position.x = gps_utm(0);
    pose_stamped.pose.position.y = gps_utm(1);
    pose_stamped.pose.position.z = gps_utm(2);
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void addRTKFactor() {
    ...
    if (poseCovariance(3, 3) < poseCovThreshold &&
        poseCovariance(4, 4) < poseCovThreshold) {
      std::cout << "pose covariance : " << poseCovariance(3, 3) << " : "
                << poseCovariance(4, 4) << std::endl;
      return;
    }
    ...
    while (!rtkQueue.empty()) {
      if (rtkQueue.front().timestamp < timeLaserInfoCur - 0.2) {
        rtkQueue.pop_front();
      } else if (rtkQueue.front().timestamp > timeLaserInfoCur + 0.2) {
        break;
      } else {
        RTKMsg this_rtk = rtkQueue.front();
        rtkQueue.pop_front();

        float noise_x = this_rtk.covariance(0, 0);
        float noise_y = this_rtk.covariance(1, 1);
        float noise_z = this_rtk.covariance(2, 2);

        float gps_x = this_rtk.pose(0);
        float gps_y = this_rtk.pose(1);
        float gps_z = this_rtk.pose(2);
        ...
        std::cout << "Add RTK GPS Factor" << std::endl;
