  ...
  bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg) {
    ...
    if (sensor == SensorType::VELODYNE) {
      ...
      for (size_t i = 0; i < tmpRsLiDARCloudIn->points.size(); i++) {
        ...
        PointXYZIRT p;
        p.x = tmpRsLiDARCloudIn->points[i].x;
        p.y = tmpRsLiDARCloudIn->points[i].y;
        p.z = tmpRsLiDARCloudIn->points[i].z;
        p.intensity = tmpRsLiDARCloudIn->points[i].intensity;
        p.ring = tmpRsLiDARCloudIn->points[i].ring;
        p.time = tmpRsLiDARCloudIn->points[i].timestamp - scan_timestamp;
        if (p.time < 0) std::cout << p.time << std::endl;
      ...
      cloudHeader = currentCloudMsg.header;
      cloudHeader.stamp.fromSec(scan_timestamp);
    } else {
